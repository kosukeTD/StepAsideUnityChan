...
public class UnityChanController : MonoBehaviour {
	...
	//Q.早すぎる・・・。設定あってる？ →UnityChanのAnimatorのApply Root Motionにチェックを入れたらなおった
	//Unityちゃんを移動させるコンポーネントを入れる
	private Rigidbody myRigidbody;
	//前進するための力（追加）Q.800.0f早すぎる・・・。→Apply Root Motion
	private float forwardForce = 800.0f;
	//左右に移動するための力 Q.500.0fだと早すぎる・・・。→Apply Root Motion
	...
	void Update () {
		...
		//Q.GetKeyDownではなぜないの？　→押した瞬間にちょっと動くだけだから
		//Q.制限がかからない・・・。行き過ぎる・・・。→Apply Root Motion
